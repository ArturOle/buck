...
@click.pass_context
def list_all(ctx):
    print(buck.list_all_in_bucket(ctx.obj["bucket_name"], ctx.obj["directory"]))
...
def upload(ctx, file_path):
    return buck.upload_file_to_bucket(
        file_path,
        ctx.obj["bucket_name"],
        ctx.obj["directory"]
    )
...
def delete(ctx, pattern):
    return buck.delete_matching_files(
        ctx.obj["bucket_name"],
        ctx.obj["directory"],
        pattern
    )
